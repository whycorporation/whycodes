...
**Status:** not started · **Depends on:** 4 (shares the CI budget harness)
...
- [ ] `scripts/bench_startup.py`: launch to first rendered frame, N runs,
      report median and p95, discard the first run as cache-warming
- [ ] `scripts/bench_memory.py`: peak RSS for a scripted session, plus the
      per-session increment
- [ ] `scripts/count_idle_draws.py`: redraws in 10s with no input; the target
      is zero
- [ ] Structured token accounting on every turn, surfaced by `/info` and
      `whycode stats`
- [ ] `docs/benchmarks.md` recording the method precisely enough to reproduce:
...
- [ ] Record the first measured numbers in `docs/comparison.md`

## Acceptance criteria

- [ ] Each benchmark is reproducible: two runs on the same machine and build
      agree within 10% on the median
- [ ] The method doc is specific enough for someone else to reproduce it
- [ ] A deliberate 2× startup regression fails the CI gate
- [ ] Idle redraws over 10 seconds with no input is 0, or the number is
