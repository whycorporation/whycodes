...
for crate in whycode-function whycode-schema whycode-skill whycode-sandbox \
             whycode-protocol whycode-plugin whycode-command-risk whycode-storage; do
...
|---|---|---|
| Workspace | **77%** lines | Every crate, including tests in the same `.rs` files |
| `function`, `schema`, `skill`, `sandbox`, `protocol`, `plugin`, `command-risk`, `storage` | **100%** lines | Production files only (`tests.rs` ignored) |
