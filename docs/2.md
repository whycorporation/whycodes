...
**Status:** not started · **Depends on:** nothing · **Blocks:** 3
...
- [ ] `release.yml`: on tag `v*`, build release binaries for
      `ubuntu-latest`, `macos-latest`, `windows-latest`
- [ ] Build both macOS architectures (`aarch64-apple-darwin`,
      `x86_64-apple-darwin`) — Apple silicon is the common case now
- [ ] Emit `SHA256SUMS` and attach every artifact to the GitHub release
- [ ] `scripts/install.sh`: detect OS and architecture, download, verify
      checksum, install to `~/.local/bin`, warn if it is not on `PATH`
- [ ] `scripts/install.ps1`: the same for Windows, installing under
      `%LOCALAPPDATA%\Programs\whycode`
- [ ] `scripts/uninstall.sh` and `scripts/uninstall.ps1`
- [ ] Implement `cmd_upgrade` against the GitHub releases API, with atomic
      replacement (download to a temp path, verify, rename over the target)
- [ ] `--version` reports version, short commit hash and build date
- [ ] README: install section leads with the scripts, source build second
