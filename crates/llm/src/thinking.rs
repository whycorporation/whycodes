    ...
    pub fn to_anthropic_value(&self) -> serde_json::Value {
        if self.enabled {
            crate::json_value::obj([
                ("type", crate::json_value::str("enabled")),
                ("budget_tokens", serde_json::Value::from(self.budget_tokens)),
            ])
        } else {
            crate::json_value::obj([("type", crate::json_value::str("disabled"))])
        }
    }
    ...
    pub fn apply_anthropic(
        ...
        if cfg.enabled {
            ...
            if current_max < min_max {
                body["max_tokens"] = serde_json::Value::from(min_max);
            }
        }
    }
