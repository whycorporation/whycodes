...
mod tests {
    ...
    fn from_base_blank_keeps_cloud_and_override_normalizes() {
        let cloud = TogetherProvider::from_base(Some("   "));
        assert!(cloud.default_base_url().contains("together"));
        let local = TogetherProvider::from_base(Some("http://127.0.0.1:9/v1"));
        assert!(
    ...
    fn build_body_without_tools_applies_sampling_and_effort() {
        let body = TogetherProvider::new().build_body(&req(), "together-model");
