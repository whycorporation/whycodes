...
fn ollama_body_uses_options_and_system_message() {
    ...
    let ollama_url = p.default_base_url();
    assert!(ollama_url.ends_with("/api/chat"), "{ollama_url}");
}
...
async fn openai_compat_wrappers_complete_and_stream_against_loopback() {
    ...
    let groq_err =
        groq::GroqProvider::from_base(Some(&serve_once("401 Unauthorized", "nope", "text/plain")));
    ...
    let mi_ok =
        mistral::MistralProvider::from_base(Some(&serve_once("200 OK", &json, "application/json")));
    ...
    let copilot_ok =
        copilot::CopilotProvider::from_base(Some(&serve_once("200 OK", &json, "application/json")));
...
async fn google_complete_and_stream_against_loopback() {
    ...
    let provider =
        google::GoogleProvider::from_base(Some(&serve_once("200 OK", &json, "application/json")));
    let req = req();
    let resp = provider
...
async fn anthropic_complete_and_stream_against_loopback() {
    ...
    assert!(
        resp.content
            .iter()
            .any(|b| matches!(b, ContentBlock::Text { text } if text.contains("hello-anthropic"))),
        "{resp:?}"
    );
...
async fn ollama_complete_and_stream_against_loopback() {
    ...
    let provider =
        ollama::OllamaProvider::from_base(Some(&serve_once("200 OK", &json, "application/json")));
    let req = req();
...
async fn openai_compat_connect_json_and_stream_http_errors() {
    ...
    let err = groq::GroqProvider::from_base(Some(&stream_err))
        .stream(&req, "k", "m")
        .await
        .map(|_| ())
        .unwrap_err();
    ...
    let err = openrouter::OpenRouterProvider::from_base(Some(&serve_once(
        "401 Unauthorized",
        "nope",
        "text/plain",
    )))
    ...
    let err = copilot::CopilotProvider::from_base(Some(&serve_once(
        "401 Unauthorized",
        "nope",
        "text/plain",
    )))
    ...
    let err = deepseek::DeepSeekProvider::from_base(Some(&serve_once(
        "401 Unauthorized",
        "nope",
        "text/plain",
    )))
    ...
    let err = mistral::MistralProvider::from_base(Some(&serve_once(
        "401 Unauthorized",
        "nope",
        "text/plain",
    )))
    ...
    let err = together::TogetherProvider::from_base(Some(&serve_once(
        "401 Unauthorized",
        "nope",
        "text/plain",
    )))
    ...
    let err =
        xai::XaiProvider::from_base(Some(&serve_once("401 Unauthorized", "nope", "text/plain")))
            .stream(&req, "k", "m")
            .await
            .map(|_| ())
            .unwrap_err();
    ...
    for err in [
        groq::GroqProvider::from_base(Some(dead))
            .stream(&req, "k", "m")
            .await
            .map(|_| ()),
        together::TogetherProvider::from_base(Some(dead))
            .complete(&req, "k", "m")
            .await
            .map(|_| ()),
        together::TogetherProvider::from_base(Some(dead))
            ...
            .await
            .map(|_| ()),
        deepseek::DeepSeekProvider::from_base(Some(dead))
            .complete(&req, "k", "m")
            .await
            .map(|_| ()),
        deepseek::DeepSeekProvider::from_base(Some(dead))
            ...
            .await
            .map(|_| ()),
        mistral::MistralProvider::from_base(Some(dead))
            .complete(&req, "k", "m")
            .await
            .map(|_| ()),
        mistral::MistralProvider::from_base(Some(dead))
            ...
            .await
            .map(|_| ()),
        openrouter::OpenRouterProvider::from_base(Some(dead))
            .complete(&req, "k", "m")
            .await
            .map(|_| ()),
        openrouter::OpenRouterProvider::from_base(Some(dead))
            ...
            .await
            .map(|_| ()),
        copilot::CopilotProvider::from_base(Some(dead))
            .complete(&req, "k", "m")
            .await
            .map(|_| ()),
        copilot::CopilotProvider::from_base(Some(dead))
            ...
            .await
            .map(|_| ()),
        xai::XaiProvider::from_base(Some(dead))
            .complete(&req, "k", "m")
            .await
            .map(|_| ()),
        openai::OpenAiProvider::from_base(Some(dead))
            .complete(&req, "k", "m")
            .await
            .map(|_| ()),
        openai::OpenAiProvider::from_base(Some(dead))
            ...
            .await
            .map(|_| ()),
        ollama::OllamaProvider::from_base(Some(dead))
            .complete(&req, "k", "m")
            .await
            .map(|_| ()),
        ollama::OllamaProvider::from_base(Some(dead))
            ...
            .await
            .map(|_| ()),
        google::GoogleProvider::from_base(Some(dead))
            .stream(&req, "k", "gemini")
            .await
            .map(|_| ()),
        anthropic::AnthropicProvider::from_base(Some(dead))
            .complete(&req, "k", "claude")
            .await
            .map(|_| ()),
    ] {
    ...
    let custom_dead = custom::CustomProvider::new(
        "custom",
        custom::normalize_chat_completions_url(dead),
        Some("sk".into()),
    ...
    let _ = custom_dead.stream(&req, "k", "m").await.map(|_| ());

    for err in [
        together::TogetherProvider::from_base(Some(&serve_once(
            "200 OK",
            "not-json",
            "text/plain",
        )))
        .complete(&req, "k", "m")
        .await,
        deepseek::DeepSeekProvider::from_base(Some(&serve_once(
            "200 OK",
            "not-json",
            "text/plain",
        )))
        .complete(&req, "k", "m")
        .await,
        mistral::MistralProvider::from_base(Some(&serve_once("200 OK", "not-json", "text/plain")))
            .complete(&req, "k", "m")
            .await,
        openrouter::OpenRouterProvider::from_base(Some(&serve_once(
            "200 OK",
            "not-json",
            "text/plain",
        )))
        .complete(&req, "k", "m")
        .await,
        copilot::CopilotProvider::from_base(Some(&serve_once("200 OK", "not-json", "text/plain")))
            .complete(&req, "k", "m")
            .await,
        xai::XaiProvider::from_base(Some(&serve_once("200 OK", "not-json", "text/plain")))
            .complete(&req, "k", "m")
            .await,
        openai::OpenAiProvider::from_base(Some(&serve_once("200 OK", "not-json", "text/plain")))
            .complete(&req, "k", "m")
            .await,
        ollama::OllamaProvider::from_base(Some(&serve_once("200 OK", "not-json", "text/plain")))
            .complete(&req, "k", "m")
            .await,
    ] {
        assert!(!err.unwrap_err().to_string().is_empty());
    ...
    let custom_parse = custom::CustomProvider::new(
        "custom",
        custom::normalize_chat_completions_url(&serve_once("200 OK", "not-json", "text/plain")),
        Some("sk".into()),
        std::collections::HashMap::new(),
    );
    assert!(
    ...
    let mut or_none = openrouter::OpenRouterProvider::from_base(Some(&serve_sse(&sse_hello())));
    or_none.site_url = None;
    or_none.site_name = None;
    assert_stream_hello(&or_none).await;

    let or_site = openrouter::OpenRouterProvider::from_base(Some(&serve_sse(&sse_hello())))
        .with_site("https://example.test".into(), "WhyCodes".into());
    assert_stream_hello(&or_site).await;
}
...
async fn anthropic_and_google_stream_http_errors() {
    let req = req();
    let err = anthropic::AnthropicProvider::from_base(Some(&serve_once(
        "401 Unauthorized",
        "nope",
        "text/plain",
    )))
