...
fn resolve_relative_path_endpoint() {
    let u = resolve_endpoint_url("http://localhost:3000/sse", "/messages?sessionId=abc").unwrap();
...
fn resolve_endpoint_handles_relative_urls_and_clears_base_query() {
    ...
    let rooted = resolve_endpoint_url("http://localhost:3000/sse?old=1", " /messages ").unwrap();
...
fn extract_result_accepts_plain_json_and_reports_rpc_error() {
    let result =
        extract_jsonrpc_result_from_sse(r#"{"jsonrpc":"2.0","id":3,"result":{"pong":true}}"#, 3)
            .unwrap();
...
async fn streamable_http_covers_json_sse_session_and_errors() {
    ...
    let err = StreamableHttpTransport::new(&url, &HashMap::new())
        ...
        .await;
    drop(err);
    let mut dead = StreamableHttpTransport::new("http://127.0.0.1:1/", &HashMap::new()).unwrap();
    let err = dead
