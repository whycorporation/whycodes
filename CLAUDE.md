...
The 26 Rust crates follow a checked, one-way dependency graph:
...
- **Foundations** own narrow infrastructure and leaf concepts: `core`, `auth`, `config`, `storage`, `protocol`, `format`, `schema`, `sandbox`, `lsp`, `skill`, `function`, `command-risk`, `slop`, `index`, and `import`.
