...
Простой TUI, Linux x86_64, 2026-09-02 (метод и машина в
[docs/benchmarks.md](docs/benchmarks.md)):

| Метрика | Результат |
|---|---|
| 1 сессия PSS | **10.5 MB** |
| 10 сессий PSS | **32.0 MB** (~2.4 MB каждая дополнительная) |
| `--version` | **1.4 ms** |
| Первый кадр (harness, in-proc) | **12 ms** |
| Перерисовки в простое (harness, 3 с) | **0.3 /s** |

TUI рисует только при изменениях. В этом прогоне 3 с простоя harness —
**0.3 перерисовок/с** (как у `ea098af` / `50e05d8`; пороги idle-zero не
вернули жёсткий ноль); целевой показатель продукта по-прежнему
**0 перерисовок/с**, а не гонка FPS.
