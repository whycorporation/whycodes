...
The TUI paints only when something changed. This Windows run’s 3 s harness idle is **0.0 redraws/s** (Linux 2026-09-02 was 0.3/s); the product target is still **0 redraws/s**, not a frames-per-second race. First paint (splash) now happens before `TuiApp` / `Config` on Linux, macOS, and Windows; Agent/SQLite/`git` hydrate after `record_draw`.
