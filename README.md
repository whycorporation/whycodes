...
| First frame (harness, in-proc) | **52 ms** (console inherit) | **12 ms** (80×24 PTY) |
| Idle redraws (harness, 3 s) | **0.0 /s** | **0.3 /s** |

The TUI paints only when something changed. This Windows run’s 3 s harness idle is **0.0 redraws/s** (Linux 2026-09-02 was 0.3/s); the product target is still **0 redraws/s**, not a frames-per-second race. Windows first-frame dropped from 131 ms → 86 ms → 56 ms → **52 ms** the same day (issue #85): clap-free `run -d`, Tokio-free splash paint, then Agent/SQLite/`git` after `record_draw`.
